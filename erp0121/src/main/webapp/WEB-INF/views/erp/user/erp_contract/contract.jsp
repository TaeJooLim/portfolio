...
<!-- 읽기 권한 처리 -->
<c:if test="${sessionScope.empAuth ne 10 && sessionScope.empDepartment ne '700'}">
	<script>
		alert("영업과 외의 사용자는 접근할 수 없는 페이지입니다.\n관리자에게 문의하세요.");
		history.go(-1);
	</script>
</c:if>
<!-- 읽기 권한 처리 -->
 <div class="page_wrap bg_white noto font_14">
                <div class="main_right_title p_10" style="height: 50px; border-bottom: 1px solid #ccc;">
                    <h2 class="font_18">영업관리<i class="fas fa-chevron-right p_lr10"></i>2. 계약서입력</h2>
                </div>
                <div class="main_right_info" style="padding: 30px 20px;">
                    <div class="table_wrap_box" style="padding-bottom: 30px;">
                        ...
                        <form action="${pageContext.request.contextPath}/user/contract_insert" name="frm" id="frm" method="post" >                         
                        <table class="input_list">
                            <tr>
                                ...
                                <td class="p_lr3">
                                    <input type="text" name="contDepartName" maxlength="50" id="contDepartName" class="input_100" value="${sessionScope.empDepartmentName}" required readonly />
                                </td>
                                <td class="bg_gray weight_700 td_7 center">담당자 직급</td>
                                <td class="p_lr3">
                                    <input type="text" name="contPositionName" maxlength="50" id="contPositionName" class="input_100" value="${sessionScope.empPositionName}" required readonly />
                                </td>
                                <td class="bg_gray weight_700 td_7 center">담당자 이름</td>
                                <td class="p_lr3">
                                    <input type="text" name="contEmpName" maxlength="50" id="contEmpName" class="input_100" value="${sessionScope.empName}" required readonly />
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="p_lr3">
                                    <input type="text" name="contClientCeoName" maxlength="50" id="clientCeoName" class="readonly input_100" required value="${cOne.clientCeoName}" style="caret-color: transparent !important;" placeholder="거래처를 선택해주세요." readonly/>
                                </td>
                                <td class="bg_gray weight_700 td_7 center">거래처 담당자</td>
                                <td class="p_lr3">
                                    <input type="text" name="contClientResponsibility" maxlength="30" id="clientResponsibility" class="readonly input_100" required placeholder="거래처를 선택해주세요." style="caret-color: transparent !important;" readonly/>
                                </td>
                                <td class="bg_gray weight_700 td_7 center">거래처 연락처</td>
                                <td class="p_lr3">
                                    <input type="text" name="contClientResCp" maxlength="30" id="clientCompanyPhone" class="readonly input_100" required placeholder="거래처를 선택해주세요." style="caret-color: transparent !important;" readonly/>
                                </td>
                                <td class="bg_gray weight_700 td_7 center">거래처 이메일</td>
                                <td class="p_lr3">
                                    <input type="text" name="contClientResEmail" maxlength="30" id="clientResEmail" class="readonly input_100" required placeholder="거래처를 선택해주세요." style="caret-color: transparent !important;" readonly/>
                            ...
                            <tr>
                                ...
                                <td class="p_lr3">
                                    <input type="text" name="clientLicenseNumFk" maxlength="20" id="clientLicenseNum" class="readonly input_100" required placeholder="거래처를 선택해주세요." style="caret-color: transparent !important;" readonly/>
                                </td>
                    ...
                    <div class="m_t10">
                        ...
                        <form id="searchfrm" name="searchfrm" method="post" action="${pageContext.request.contextPath}/user/contract_detail_prod">
	                        <input type="hidden" value="${contID}" id="contID" name="contID"/>
	                        <input type="hidden" value="${contDocNum}" id="contDocNum" name="contDocNum"/>
	                                    ...
	                                    <tbody class="plist">
		                                    ...
		                                    <c:forEach var="p" items="${pList}" varStatus="status">
		                                        <tr>
		                                            ...
		                                            <td>${p.pModelnum}</td>
		                                            <td>${p.pBrand}</td>
		                                            <td> <button type="button" id="" class="proc_sel_btn s_btn_gray" onclick="location.href='${pageContext.request.contextPath}/user/contract_detail_list?contID=${contID}&contDpName=${p.pName}'">선택</button> </td>
		                                        </tr>
		                                    </c:forEach>    
	                                    </tbody>
                        ...
                        <table class="user_table_list m_t20">
                            <thead>
                                <tr>
                                    <th class="td_5">번호</th>
                                    <th>문서번호</th>
...
<script>
$("#contOrderDate, #contDueDate").click(function() {
...
function conRegBtn(){
	...
	$.ajax({
			...
			success	: function(resdata) {
				alert( "문서번호가 생성되었습니다.");
				window.location.href="/erp/user/contract_detail?contID="+resdata;
...
function procSel(contID, pName){
	window.location.href="/erp/user/contract_detail_list?contID="+contID+"&contDpName="+pName;
};
