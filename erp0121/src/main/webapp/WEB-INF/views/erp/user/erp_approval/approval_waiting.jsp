...
<c:if
	...
	<script>
		alert("인사과 부서장 및 총무과 부서장 외의 사용자는 접근할 수 없는 페이지입니다.\n관리자에게 문의하세요.");
		history.go(-1);
	</script>
</c:if>
<!-- 읽기 권한 처리 -->
	<div class="page_wrap bg_white noto font_14">
        <div class="main_right bg_white" style="width: 100%;">
            <div class="main_right_title p_10" style="height: 50px; border-bottom: 1px solid #ccc;">
                <h2 class="font_18">전자결재관리<i class="fas fa-chevron-right p_lr10"></i>1. 미결함</h2>
            </div>
        </div>
        <div class="main_right_info" style="padding: 30px 20px;">
            ...
            <div class="search_wrap">
	            	<div class="total_cnt_btn_grp flex">
	            		<c:choose>
	            			<c:when test="${appType eq 'V'}">
			            		<a href="${pageContext.request.contextPath}/user/appWaiting?empId=${sessionScope.empId}">
		                            <button type="button" class="rad_btn_blue m_r5">휴가신청서</button>
		                        </a>
	            			</c:when>
	            			<c:otherwise>
	            				<a href="${pageContext.request.contextPath}/user/appWaiting?empId=${sessionScope.empId}&appType=O">
	                ...
	                <form id="" name="" method="post" action="${pageContext.request.contextPath}/user/appWaiting?appType=${appType}&empId=${sessionScope.empId}">
                    	...
                    	<c:forEach items="${List}" var="list" varStatus="status">
	                        <tr>
	                            ...
	                            <td>
	                           		<c:choose>
	                           			<c:when test="${appType eq 'V'}">
			                           		<button type="button" class="s_btn_red"  onClick="updateConfirm('${list.orderDocNum}');">결재</button>
	                           			...
	                           			<c:otherwise>
			                           		<button type="button" class="s_btn_red" onClick="signConfirm('${list.orderDocNum}');">결재</button>
...
<script>
	var curHref = "/erp/user/appwaiting?empId=${sessionScope.empId}";
