...
<!-- 읽기 권한 처리 -->
<c:if
	test="${sessionScope.empAuth ne 10 && sessionScope.empDepartment ne '400'}">
	<script>
		alert("총무과 전용입니다.\n관리자에게 문의하세요.");
		history.go(-1);
	</script>
            ...
            <form id="carReg" name="carReg" method="POST" action="${pageContext.request.contextPath}/user/corp_car_management_insert">
                <div class="input_list w_100 white_s_no">
                    ...
                    <div class="btn-grp center m_t10">
                    			<button type="button" id="carchangebtn" name="carchangebtn" class="btn_blue" style="background-color: #0099a4;">차량수정</button>
	                        	<button type="submit" id="carRegBtn" class="btn_blue" style="background-color: #0099a4;">차량등록</button>
	                        	<button type="reset" id="cancel" class="btn_ivory">취소</button>
	                        	<button type="button" id="back" onClick="window.location.reload();" class="btn_red" style="background-color: #00136f;">수정취소</button>
                    </div>
                </div>
            </form>
                        ...
                        <tbody>
                        	<c:forEach var="car" items="${list}" varStatus="status">
	                            <tr>
	                                ...
	                                <td>
	                                    <button class="s_btn_blue" style="background-color: #148cb2;"
	                                     onClick="location.href='${pageContext.request.contextPath}/user/corp_car_log?carID=${car.carID}'">일지작성</button>
	                                    <button class="s_btn_red" style="background-color: #00136f;" type=button id="updateBtn" onclick="updateBtn('${car.carID}');">수정</button>
	                                </td>
	                            </tr>
	                         </c:forEach>   
                        </tbody>
