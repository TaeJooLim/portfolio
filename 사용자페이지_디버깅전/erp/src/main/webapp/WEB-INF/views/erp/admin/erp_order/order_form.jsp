	...
	<div class="page_wrap noto font_12">
		<div class="print_btn_grp flex flex_fe">
			...
			<c:if test="${orderConfirm eq 'N' }">
				<button type="button" id="sign_btn" class="m_r5 btn_red" style="outline:none" onClick='signConfirm();'>결재</button>
			</c:if>
		</div>
...
<script>
	function signConfirm() {
		if(confirm("결재를 진행하시겠습니까?")) {
			$.ajax({
				url 		: "${pageContext.request.contextPath}/admin/orderSignConfirm",
				type		: "post",
				data 		: { orderDocNum : $("#orderDocNum").html() },
				success 	: function(resData) {
								alert("결재가 완료되었습니다.");
								window.location.reload();
				},
				error 		: function() {
								alert("결재확인 AJAX 장애발생\n관리자에게 문의해주세요");
				}
			});
		}
	};
