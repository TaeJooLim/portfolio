...
public class AttendDaoImpl implements AttendDao{
	...
	public void setEndtime(AttendVO avo) {
		...
		// 6. 초과근무시간 salary에 update
		String chkDate = avo.getAttendWorkdate().substring(0, 7);
		int salaryOvertimePay = overtime;
		if (overtime < 0) salaryOvertimePay = 0;
		else salaryOvertimePay = (overtime / 60) * 10000;
		
		map.put("salaryDate", chkDate);
		map.put("empId", avo.getEmpIdFk());
		map.put("salaryOvertimePay", salaryOvertimePay);
		
		sqlSession.update("salary.changeOvertimePay", map);
	...
	public List<AttendVO> getWorkDatas(String searchOpt, String words,int startIndex, int pageSize) {
		...
		return sqlSession.selectList("attend.getWorkDatas",map);
	}
	...
	public int isWorkDatas(String searchOpt, String words) {
		...
		return sqlSession.selectOne("attend.isWorkDatas",map);
	}
	...
	public void changeWorkplace(AttendVO avo) {
		sqlSession.update("attend.changeWorkplace", avo);
	}

	@Override
	public void changeStatus(AttendVO avo) {
		sqlSession.update("attend.changeStatus", avo);
	}

	@Override
	public void deleteRecords(int attendId) {
		sqlSession.delete("attend.deleteRecords", attendId);
	}

	@Override
	public AttendVO getRecordOne(AttendVO avo) {
		return sqlSession.selectOne("attend.getRecordOne", avo);
	}

	@Override
	public AttendVO getDefault(AttendVO avo) {
		return sqlSession.selectOne("attend.getDefault", avo);
	}

	@Override
	public void setChanges(AttendVO avo) {
		sqlSession.update("attend.setChanges", avo);
		
		/*	출근 퇴근시간 가져와서 초과근무시간/총근무시간 계산 후, mapper로 돌아가서 update set초과근무시간/총근무시간해주기	*/
		
		// 1. 초과업무시간(분) = 총업무시간 - 소정근무시간(default 540분)
		AttendVO getOvertime = sqlSession.selectOne("attend.getRecordOne", avo.getAttendId());
		int overtime = getOvertime.getAttendOvertime();
		
		// 2. 초과근무시간 salary에 update
		String chkDate = avo.getAttendWorkdate().substring(0, 7);
		int salaryOvertimePay = (overtime/60)*10000;
	...
	@Override
	public List<AttendVO> getAttendListStatus(int status) {
		return sqlSession.selectList("attend.getAttendListStatus", status);
	}
	...
	public List<AttendVO> getMyAttend(AttendVO avo,String searchOpt, String words,int startIndex,int pageSize) {
		...
		return sqlSession.selectList("attend.getMyAttend", map);
	}
	...
	public int countMyAttend(AttendVO avo,String searchOpt, String words) {
		...
		map.put("searchOpt", searchOpt);
		map.put("words", words);
		return sqlSession.selectOne("attend.countMyAttend", map);
	}

	@Override
	public int countStatusTwo(AttendVO avo) {
		return sqlSession.selectOne("attend.countStatusTwo", avo);
	}

	@Override
	public int countStatusOne(AttendVO avo) {
		return sqlSession.selectOne("attend.countStatusOne", avo);
	}

	@Override
	public int countLateDays(AttendVO avo) {
		return sqlSession.selectOne("attend.countLateDays", avo);
	}
}