...
<link rel="stylesheet" href="${pageContext.request.contextPath}/css/user_button_style.css">
    <div class="page_wrap bg_white noto font_14">
        ...
        <div class="main_right bg_white">
            <div class="main_right_title p_10" style="height: 50px; border-bottom: 1px solid #ccc;">
                <h2 class="font_18">근태관리<i class="fas fa-chevron-right p_lr10"></i>4. 휴가사용이력조회</h2>
            </div>
        </div>
        <div class="main_right_info" style="padding: 30px 20px;">
        	<div class="table_wrap_box">
                <div class="search_wrap">
	            	<div class="total_cnt_btn_grp flex">
	            		<a href="${pageContext.request.contextPath}/user/vacation_list">
		                	<button class="total_list_btn m_r5">전체목록</button>
	            		</a>
	                	<div class="search_cnt_btn_grp flex">
	                    	<button class="left_radius_btn white">검색 결과</button>
	                		<button class="right_radius_btn">${vCnt}개</button>
	                	</div>
	                </div>
	                <form id="" name="" method="post" action="${pageContext.request.contextPath}/user/vacation_list">
            ...
            <div class="m_t10">
                <div class="w_100 white_s_no overflow_x_auto">
                    ...
                    <div class="m_b5">
                        <div class="tbl_wrap">
                            <table class="user_table_list clearfix m_b30">
                                <thead>
                                	<tr>
                                		<th class="td_5">번호</th>
	                                    <th>문서번호</th>
                                ...
                                <tbody>
                                	...
                                	<c:forEach items="${vList}" var="list" varStatus="status">
		                                <tr>
		                                	<td>${(vCnt - status.index) - ((pagingvo.curPage - 1) * pagingvo.pageSize)}</td>
		                                    ...
		                                    <td>
		                                    	<c:choose>
		                                    		...
		                                    		<c:otherwise>
		                                    			<button type="button" class="s_btn_red"  onClick="updateConfirm('${list.offDocNum}');" >결재</button>
		                                    		</c:otherwise>
		                                    	</c:choose>
	                                    		<button type="button" class="s_btn_gray" onClick="window.open('${pageContext.request.contextPath}/admin/vacationForm?offDocNum=${list.offDocNum}')">보기</button>
		                                    </td>
		                                    <td>
		                                    	<c:choose>
		                                    		<c:when test="${list.offConfirm eq 'Y'}">
    ...
    <script>
    	var curHref = "/erp/user/vacation_list";
