                ...
                <div class="main_right_info flex">
                    ...
                    <div class="board_right p_20">
                        ...
                        <div class="flex space_between m_b5">
                            <div class="p_r2 flex" style="white-space: nowrap;">
	                            <c:if test="${sessionScope.empAuth eq 10}">
	                                <button class="rad_btn_red m_r5" id="deleteAll" onclick="deleteAll('${bvo.boardNum}');" >선택삭제</button>
	                            </c:if>
                                <div class="search_cnt_btn_grp flex">
                                    <button class="left_radius_btn white">검색된 사원 수</button>
                                    <button class="right_radius_btn">${listCnt}명</button>
                                </div>
                            </div>
	                        <form class="" method="post" action="${pageContext.request.contextPath}/user/board_list?boardNum=${bvo.boardNum}">
                        ...
                        <div class="w_100 white_s_no overflow_x_auto">
                                ...
                                <c:forEach items="${alist}" var="alist" varStatus="status">
                                    <tr>
                                        ...
                                        <td class="p_l10" style="text-align:left;">
                                            <a href="${pageContext.request.contextPath}/user/board_view?boardNum=${bvo.boardNum}&aId=${alist.aId}" class="weight_700 m_r5 subject">${alist.subject}</a>
                                        ...
                                        <td>
                                        	<c:choose>
	                                        	<c:when test="${sessionScope.empAuth eq 10 || sessionScope.empId eq alist.empIdFk}">
	                                        		<button class="s_btn_blue" type="button" onclick="location.href='${pageContext.request.contextPath}/user/board_modify?boardNum=${bvo.boardNum}&aId=${alist.aId}'">수정</button>
	                                       		</c:when>
	                                       		<c:otherwise>
	                                       			-
	                                       		</c:otherwise>
                            ...
                            <c:if test="${not empty alist}"><div class="m_b30"></div></c:if>
	                        <div class="flex space_between">
	                            <div style="margin: 0 auto;">
	                            	<%@ include file="/WEB-INF/views/erp/user/include/USER_PAGING.jsp" %>
	                            </div>
	                            <div class="">
	                                <button class="btn_write white_s_no" type="button" onclick="location.href='${pageContext.request.contextPath}/user/board_write?boardNum=${bvo.boardNum}'" >글쓰기</button>
	                            </div>                           
	                        </div>
                           	<c:if test="${empty alist}"><div class="m_b30"></div></c:if>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
	var curHref = "/erp/user/board_list";
