...
public class UserMainCtr {
	...
	public ModelAndView getUserMain(@ModelAttribute EmployeeVO evo) {
		...
		// 나의 쪽지함(받은메시지 보관함과 연결)
		String msgWords = "";
		String msgSearchOpt = "msg_subject";
		mav.addObject("mUnreadList", messageSrv.getFromUnreadList(empId, msgSearchOpt, msgWords)); // 받은메시지 목록
		mav.addObject("mUnreadCnt", messageSrv.getFromUnreadCnt(empId, msgSearchOpt, msgWords)); // 읽지않은 받은메시지 개수
